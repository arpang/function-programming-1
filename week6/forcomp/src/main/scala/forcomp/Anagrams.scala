...
object Anagrams {
  ...
  def sentenceOccurrences(s: Sentence): Occurrences = {
    if (s.isEmpty) List()
    else if (s.length == 1) wordOccurrences(s.head)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    y.foreach(a => require(x.count(_._1 == a._1) == 1))
    y.foreach(a => require(x.find(_._1 == a._1).get._2 >= a._2))
    y match {
      case Nil => x
      case head :: tail =>
        val (char, ycount) = head
        val xcount = x.find(_._1 == char).get._2
        if (xcount == ycount) subtract(x.filter(_._1 != char).sorted, tail)
        else subtract(((char, xcount - ycount) :: x.filter(_._1 != char)).sorted, tail)
    }
  }
